...
export default class Peaceful {
    ...
    enabled: bool_ref = [true];
    //lastroom: number = this.ModLoader.V

    @EventHandler(Z64.OotEvents.ON_ACTOR_SPAWN)
    updateEnemy(actor: IActor) {

        if (this.enabled[0] && actor.actorType == ActorCategory.ENEMY)
            actor.destroy();
    ...
    updateBitFlag(byte: number, bit: number, set: Bit) {
        if (this.isBitSet(byte, bit, set))
            return;

        let temp = this.core.OOT!.save.permSceneData;
        let tempbit = bitwise.byte.read(temp[byte] as UInt8);
        tempbit[bit] = set;
        let tempfinal = bitwise.byte.write(tempbit);
        temp[byte] = tempfinal;
        this.core.OOT!.save.permSceneData = temp;
        this.ModLoader.logger.debug("Stopping some violence!");
    }

    isBitSet(byte: number, bit: number, set: Bit) {
        let temp = this.core.OOT!.save.permSceneData;
        let tempbit = bitwise.byte.read(temp[byte] as UInt8);
        if (tempbit[bit] == set)
            return true;
